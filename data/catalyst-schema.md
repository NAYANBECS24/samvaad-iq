...
Provision these tables in the existing Catalyst project before setting `CATALYST_DATASTORE_ENABLED=true`.

| Table | Required fields | Purpose |
| --- | --- | --- |
| `Cases` | `fir_id`, `district`, `station_id`, `crime_type`, `date`, `time`, `lat`, `lon`, `status`, `mo`, `case_summary`, `bns_sections`, `accused_ids`, `vehicle`, `phone_hash`, `synthetic`, `data_label` | Synthetic FIR corpus |
...
| `AuditEvents` | `audit_id`, `actor`, `action`, `resource`, `request_id`, `previous_hash`, `hash`, `created_at` | Tamper-evident event chain |
